...
**[specs.md](https://specs.md/)** offers four "pluggable flows," chosen based on how complex the project is: an Ideation flow (Spark → Flame → Forge, diverging on ideas first, then converging into a brief before writing any formal spec); a Simple flow (spec generation without execution tracking); a FIRE flow (a flexible number of checkpoints, with explicit support for brownfield work, meaning it detects existing codebase conventions before extending them); and a full AI-DLC-flow with Domain-Driven Design and full traceability. It's markdown-based and works across Claude Code, Cursor, Copilot, Windsurf, and Cline.
