...
- QA/test authoring and documentation are the most consistently agent-favorable categories, because success is directly verifiable (tests pass/fail, docs match code).
...
- Industry surveys reporting ~93% AI-tool adoption alongside measured output gains closer to ~10% suggest a large share of adoption isn't yet translating into throughput — often because review overhead, context-setup time, and correcting agent mistakes eat into the raw generation speedup.
...
- Unreviewed agent output that turns out wrong costs more than it saved → [risk-tiering](04-governance-risk-tiers.md) keeps review time proportional to actual risk instead of blanket caution or blanket trust.
- Poor spec quality is the single biggest driver of wasted agent cycles → the ["verifiable done" test](03-phases/requirements-spec.md) exists specifically to prevent the rework loop that erases savings.
