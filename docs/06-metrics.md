...
|---|---|
| Agent-authored change ratio | % of merged changes originated by an agent, by risk tier — shows where autonomy is actually concentrated |
| Override/rejection rate | % of agent output rejected or substantially rewritten in review; a rising trend flags a spec or context problem, not a review-process problem |
...
| Defect/incident rate delta | Post-release defects attributable to agent-authored vs. human-authored changes — catches a velocity gain that's really a quality trade |
...
3. Review monthly, act on trend not noise — a single bad week from one agent workflow isn't a signal; three consecutive months of rising override rate is.
