...
Write a detailed spec first; the agent implements against it and is graded against it — the spec is the source of truth, not the conversation that produced it.

## Why it exists

Agents don't share context the way a teammate who sat in every meeting does. A vague request ("improve search") gets interpreted differently every run. A spec that passes the **verifiable done** test — a reviewer could confirm completion from the spec text alone — removes that ambiguity for both the agent and the human reviewing its output.
...
3. **Explicit constraints** — performance, security, style. These are usually invisible to an agent unless stated (a human infers "don't break the public API"; an agent won't unless it's written down).
...
5. **Out of scope.** Without this, agents "helpfully" touch adjacent code. This section is what keeps a bounded task bounded.
...
- **No out-of-scope section.** The most common cause of an agent's diff touching 30 files when you expected 3.
...
A good spec is what [evals](evals.md) get written against, what [context engineering](context-engineering.md) curates supporting material for, and what a [plan-then-execute](plan-then-execute.md) review checks the agent's plan against before execution starts.
