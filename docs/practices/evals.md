...
Test suites for agent *behavior*, written and run the same way you'd write unit tests — verifying the agent does the right thing, not just that the code compiles. Evals become part of CI, gating agent/prompt changes before they ship.

## Why it exists

A code-level test verifies the output artifact. An eval verifies the agent's *decision-making* on a class of task — which matters because the same agent workflow runs repeatedly on different inputs, and you need confidence it generalizes, not just that it got one instance right.
...
- Tests the agent wrote to match its own implementation — this can encode the same bug into both the code and the test. Evals should be authored independently of the specific implementation being graded.
- No regression tracking over time — a single green run doesn't tell you if quality is drifting across prompt/model changes.
- Treating "the agent wrote tests" as equivalent to "the behavior is verified" (see [testing-qa.md](../03-phases/testing-qa.md)).
